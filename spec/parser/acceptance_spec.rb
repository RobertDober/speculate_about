    ...
    describe "this only child" do
      ...
      it "and is correctly rendered" do
        expect( ctxt2.examples.first.to_code )
          .to eq([
            "    it \"therefore we have (spec/fixtures/ACCEPTANCE.md:35)\" do", "      a nice example # We do not check the code, RSpec will do that for us", "    end"])
      end
    end
  end
  ...
  context "leaf node" do
    ...
    it "and is correctly rendered" do
      expect( ctxt3.examples.first.to_code )
        .to eq(
          ["      it \"this shall be (spec/fixtures/ACCEPTANCE.md:23)\" do", "        expect( 1 ).to be_zero # Who cares?", "      end"])
    end
  end
